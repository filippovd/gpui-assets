...
/// Input for icon_name! macro: EnumName, "prefix", "path", [optional derives]
...
///
/// NOTE: this helper is duplicated in `crates/utils/src/lib.rs` because
/// proc-macro crates cannot depend on regular workspace crates without
/// breaking their isolation. Keep the two implementations in sync.
///
/// # Examples
///
/// ```ignore
...
///
/// Accepts an enum name, an asset prefix, a path, and optionally a list of additional
/// derive traits. Each `.svg` file becomes an enum variant using PascalCase conversion.
/// The generated asset path for each variant is `{prefix}:/{filename}`, which is
/// compatible with prefix-based asset routing (e.g. `lucide:/arrow-right.svg`).
///
...
/// icon_named!(IconName, "icons", "assets/icons");
///
/// // Env-var reference (resolved at macro expansion time)
/// icon_named!(IconName, "icons", "$GPUI_COMPONENT_DEFAULT_ICONS_DIR");
///
/// // With custom derives
/// icon_named!(IconName, "icons", "assets/icons", [Debug, Copy, PartialEq, Eq]);
/// ```
#[proc_macro]
    ...
    for entry in dir {
        ...
        if filename.ends_with(".svg") {
            ...
            if variant_name.is_empty() {
                panic!(
                    "icon_named!: '{}' produces an empty variant name after PascalCase \
                     conversion; rename the file so it has at least one alphabetic segment",
                    filename
                );
            }
            let path = format!("{}:/{}", raw_prefix, filename);
            entries.push((variant_name, path, filename));
        }
    }

    entries.sort_by(|a, b| a.0.cmp(&b.0));

    // Reject name collisions before codegen: different files that normalize to
    // the same PascalCase variant (e.g. `foo-bar.svg` and `foo_bar.svg` both
    // yield `FooBar`) would otherwise produce a duplicate-variant compile error
    // with no pointer to the cause.
    let mut seen: std::collections::HashMap<&str, &str> = std::collections::HashMap::new();
    for (variant_name, _path, filename) in &entries {
        if let Some(previous) = seen.get(variant_name.as_str()) {
            panic!(
                "icon_named!: variant name `{variant_name}` is generated by both `{previous}` and \
                 `{filename}`; rename one of the files so the PascalCase forms differ"
            );
        }
        seen.insert(variant_name.as_str(), filename.as_str());
    }
